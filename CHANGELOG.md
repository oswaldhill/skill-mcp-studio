...
- **引入 ruff 静态检查（P1-11，部分完成）**：`pyproject.toml` 新增 `[tool.ruff]`，
  并新增 CI job `lint-ruff`（`ruff==0.16.9` 固定版本）。规则集**分阶段开启**：
  当前 `select = ["E4","E7","E9","F"]`，`ignore = ["F401","E402"]` —— 后两条会改变
  代码行为（`F401` 在本项目多为有意的再导出，实测 `--fix` 破坏 5 个用例；
  `I001`/`E402` 会打乱 `sys.path` 引导顺序），需先做准备，配置内已写明原因。
  落地时清掉 190 处告警：13 处自动修、6 处死赋值、`E741` 改名，以及两处性质相反的
  `F811`（一处真冗余删除，一处刻意的惰性导入加 `noqa` 保留）。
  未完成部分：`F401`、`I001`、`ruff format`。
