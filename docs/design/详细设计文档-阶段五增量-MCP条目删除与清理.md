...
备份一并列出（本机实测：Codex 的备份里有
`config.toml.bak-memory-remediation-20260920-163013` 这类由其它流程产出的备份）。
`restore_config_backup` 用**同一前缀**做归属校验，所以「列出的都能还原」是一致的、
不构成 bug；但 UI / 文档不应把该列表暗示为「本模块自己生成的备份」——
**「列出即可控」**（列出即代表可还原）这一点需在文案中讲清。
